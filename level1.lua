...
local janken_points = 0 --  enemy points initialization 
local alex_points = 0  -- alex points initialization
local loadingBarBackground  -- time progress initialization
local loadingBar
...
function scene:create( event )  
   ...
   local a_win_msg = display.newText("Alex : 0",60,90,native.systemFont,30)   -- text to display Alex values 
    sceneGroup:insert(a_win_msg)
    local e_win_msg = display.newText("Enemy : 0",240,90,native.systemFont,30)  --text to display Enemy values
  ...
  -- diaplay the Alex kid in the screen and play the sequence 
...
--create the bubble scene sequence 
 ...
 -- diaplay the bubble sequnce in the screen and play the sequence 
     ...
     -- Create animation sequence janken's 
     local seqDataJanken = {
     ...
     local janken = display.newSprite (jankenSheet, seqEnemyData);
...
-- listener removal after completing the action
       bubble:removeEventListener("touch",bubble_listener)
        ...
        if (x ==1 and design ==12)  then -- conditions to check win for every fight between alex and enemy

                  display.remove(win_message)
                  win_message=display.newText("Draw Match!!",155,395,native.systemFont,30) --display the winning message 
                  win_message:setFillColor(1,1,0)
                  audio.play(result_sound)   --play the audio sound for winning 

        elseif (x ==2 and design ==10)  then --- conditions to check win for every fight between alex and enemy
                  display.remove(win_message)
                  win_message=display.newText("Draw Match!!",155,395,native.systemFont,30) ---display the winning message 
                  audio.play(result_sound)   ---play the audio sound for winning 

        elseif (x ==3 and design ==11)  then ---conditions to check win for every fight between alex and enemy
                  display.remove(win_message)
                  win_message=display.newText("Draw Match!!",155,395,native.systemFont,30) ---display the winning message 
                  win_message:setFillColor(1,1,0)
                  audio.play(result_sound)   ---play the audio sound for winning 

        elseif (x ==3 and design ==10)  then ----conditions to check win for every fight between alex and enemy
                  janken_points = janken_points + 1
                  print ("enemy"..janken_points)
                  display.remove(e_win_msg)
                  --displays the enemy scores
        ...
        elseif (x ==2 and design ==12)  then --conditions to check win for every fight between alex and enemy
                  janken_points = janken_points + 1
                  print ("enemy"..janken_points)
                  display.remove(e_win_msg)
                  e_win_msg = display.newText("Enemy : "..janken_points,240,90,native.systemFont,30)
                  ...
                  win_message:setFillColor(1,1,0)
                  audio.play(result_sound)
        elseif (x ==1 and design ==11)  then --conditions to check win for every fight between alex and enemy
                  janken_points = janken_points + 1
                  print ("enemy"..janken_points)
                  display.remove(e_win_msg)
                  e_win_msg = display.newText("Enemy : "..janken_points,240,90,native.systemFont,30)
        ...
        elseif (x ==3 and design ==12)  then --conditions to check win for every fight between alex and enemy
                  ...
                  win_message:setFillColor(1,1,0)
                  audio.play(result_sound)
        elseif (x ==1 and design ==10)  then --conditions to check win for every fight between alex and enemy
                  alex_points = alex_points + 1
                  print ("alex"..alex_points)
                  display.remove(a_win_msg)
                  a_win_msg = display.newText("Alex : "..alex_points,60,90,native.systemFont,30)
                  ...
                  win_message:setFillColor(1,1,0)
                  audio.play(result_sound)
        elseif (x ==2 and design ==11)  then --conditions to check win for every fight between alex and enemy
                  alex_points = alex_points + 1
                  print ("alex"..alex_points)
                  display.remove(a_win_msg)
                  a_win_msg = display.newText("Alex : "..alex_points,60,90,native.systemFont,30)
        ...
        if (( alex_points + janken_points)<3) then   --checking if 3 rounds of games have been played between two charecters
                ...
                function continueGame( event )  --adding the event listener to the continue button once touched
                    ...
                    if event.phase=="ended" then
                        ...
                        display.remove(hand)
                        -- progressive view to display time 
                            -- Create variables for the width, height, and corners of our loading bar
                            loadingWidth, loadingHeight, loadingCorners = 310, 40, 5
                            -- Create the loading bar background. This background is white and placed on the center of the screen.
                            loadingBarBackground = display.newRoundedRect(0,0,loadingWidth,loadingHeight,loadingCorners)
        ...
        if (( alex_points + janken_points) == 3) then
          if (alex_points < janken_points) then
            ...
            menu_button_text = display.newText("Go Home",155,460,native.systemFont,25)   -- go home button
            sceneGroup:insert(menu_button)
            sceneGroup:insert(menu_button_text)

            function return_menu( event ) --event listener for the go home button
                -- body
                if event.phase == "ended" then

                --- removes the display messages and sprites on the screen
                    display.remove(win_message)
            ...
            -- touch listerner for the return menu 
            menu_button:addEventListener("touch",return_menu)

          elseif(alex_points > janken_points) then
            level2_button = display.newRoundedRect(155,460,160,50,10)
            level2_button:setFillColor(1,0.3,0)
            level2_button_text = display.newText("Level2",155,460,native.systemFont,25)   -- go home button
            sceneGroup:insert(level2_button)
            sceneGroup:insert(level2_button_text)

            function level2( event ) --adding event listener for the go home button
                  if event.phase == "ended" then
            ...
            level2_button:addEventListener("touch",level2)
          end
        end
      end
     ...
     --#######################
         ...
         loadingBarBackground.x, loadingBarBackground.y = 160,-24
         -- Create the actual loading bar that will move. We'll make this loading bar a bit smaller than the background so the user can see the progress a bit easier. The xScale is set to 0.001 so it's hidden when it first loads.
         loadingBar = display.newRoundedRect(0,0,loadingWidth*0.975,loadingHeight*0.925,loadingCorners)
         sceneGroup:insert(loadingBarBackground)
         sceneGroup:insert(loadingBar)
         loadingBar.anchorX = 0
         loadingBar.x, loadingBar.y = loadingBarBackground.x - (loadingWidth * 0.4875), loadingBarBackground.y
